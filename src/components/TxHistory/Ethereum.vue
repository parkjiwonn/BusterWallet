<template>
    <div v-if="isTX" class="flex flex-col w-full px-2 overflow-auto max-h-screen">
        ...
        <div class="border border-indigo-800 mt-2 p-2" v-for="(transfer, subIndex) in transactions"
            ...
            <div class="flex flex-col w-full">
                ...
                <p class="truncate text-blue-500">Tx Hash : {{ transfer.hash }}</p>
            </div>
        </div>
    </div>
    ...
    <div v-else>
        ...
        <p v-else class="mt-10 flex flex-row justify-center items-center">{{ $t("message.Tx.noTx") }}</p>
...
export default {
    ...
    async created() {
        ...
        const data = await getSepoliaTxHistory(this.accountID, 'mainnet');
        //console.log('eth mainnet Tx History : ', data);
