...
export async function SendEth(senderPrivateKey, receiverAddress, amount) {
    try {
        const response = await fetch(`http://221.148.25.26:4000/ethreum/send`, {
            method: 'POST', // or 'PUT'
            headers: {
...
export async function CalculateGas(Ether, toAddress) {
    try {
        ...
        const response = await fetch(`http://221.148.25.26:4000/ethreum/gas?ether=${ether}&toAddress=${toAddress}`);
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        const data = await response.json();
...
export async function SendNear(senderAcountId, senderPrivateKey, receiverId, amountStr) {
    try {
        ...
        const response = await fetch(`http://221.148.25.26:4000/near/sendNear`, {
            method: 'POST', // or 'PUT'
            headers: {
                'Content-Type': 'application/json',
            },
...
export async function SendBuster(senderAccountId, senderPrivateKey, receiverID, inputAmount) {
    try {
        ...
        const response = await fetch(`http://221.148.25.26:4000/sendBoom`, {
            method: 'POST', // or 'PUT'
            headers: {
                'Content-Type': 'application/json',
            },
