...
export async function createBitTestAccount() {
    try {
        const response = await fetch('http://221.148.25.26:4000/bitcoin/createTestnet');
        if (!response.ok) {
...
export async function createNearAccount() {
    try {
        const response = await fetch('http://221.148.25.26:4000/near/createAccount');
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        const data = await response.json();
...
export async function createEthAccount() {
    try {
        ...
        const response = await fetch('http://221.148.25.26:4000/ethereum/createAccount');
